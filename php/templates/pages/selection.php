...
<div class="mx-auto max-w-5xl px-5 py-14 sm:py-20">
  ...
  <div class="text-[0.62rem] uppercase tracking-[0.24em] text-muted"><?= $de ? 'Bildauswahl' : 'Fotoğraf seçimi' ?></div>
  ...
  <?php else : ?>
    ...
    <div class="mt-8 flex flex-wrap items-center gap-6 border-y border-sand-deep py-6">
      <div>
        ...
        <div class="mt-1 text-[0.6rem] uppercase tracking-[0.2em] text-muted"><?= $de ? 'Bilder' : 'Fotoğraf' ?></div>
      </div>
      ...
      <?php if ($withOriginal !== []) : ?>
        <a href="<?= e(I18n::path('/auswahl/' . $token . '/zip', $locale)) ?>"
          ...
          <?= $de ? 'Alle herunterladen (ZIP)' : 'Hepsini indir (ZIP)' ?>
        </a>
      <?php endif; ?>
    </div>
    ...
    <?php if (count($withOriginal) < count($photos)) : ?>
      <p class="mt-5 border border-gold/50 bg-sand/40 px-5 py-3 text-[0.84rem] leading-relaxed text-ink">
        <?= $de
          ...
          : (string) count($photos) . ' fotoğraftan ' . count($withOriginal) . ' tanesi tam çözünürlükte mevcut. Kalanlar temsili görsel ya da bu değişiklikten önce yüklenmiş — onları fotoğrafçıdan isteyin.' ?>
      </p>
    <?php endif; ?>
  ...
  <?php if ((string) ($share['expires'] ?? '') !== '') : ?>
    <p class="mt-14 border-t border-sand-deep pt-6 text-[0.75rem] text-muted">
      <?= $de
        ...
        : 'Bu bağlantı ' . e(\Atelier\Dates::short((string) $share['expires'])) . ' tarihine kadar geçerlidir.' ?>
    </p>
  <?php endif; ?>
</div>
